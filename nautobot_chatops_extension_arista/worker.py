...
def check_token(dispatcher):
    if not os.getenv("NAUTOBOT_CHATOPS_CVP_TOKEN"):
        dispatcher.send_warning("NAUTOBOT_CHATOPS_CVP_TOKEN not set.")
        return False
    return True
...
def get_devices_in_container(dispatcher, container_name=None):
    ...
    if not container_name:
        ...
        dispatcher.prompt_from_menu(f"cloudvision get-devices-in-container", "Select Container", choices)
        return False
...
def get_configlet(dispatcher, configlet_name=None):
    """Get configlet configuration from Cloudvision"""
...
def get_device_configuration(dispatcher, device_name=None):
    ...
    if not device_name:
        ...
        dispatcher.prompt_from_menu(f"cloudvision get-device-configuration", "Select Device", choices)
...
def get_task_logs(dispatcher, task_id=None):
    ...
    if not task_id:
        ...
        dispatcher.prompt_from_menu(f"cloudvision get-task-logs", "Select task", choices)
...
def get_applied_image_bundles(dispatcher, filter_type=None, image_bundle_name=None):
    ...
    if filter_type == "all":
        ...
        # For getting image bundles. API call currently not working
        ...
        rows = [(image["name"], image["imageSize"]) for image in images]
...
@subcommand_of("cloudvision")
def get_device_bugs(dispatcher, device_name=None):
    """Get bugs Cloudvision has found for a device."""
    ...
    if not device_name:
        ...
        dispatcher.prompt_from_menu(f"cloudvision get-device-bugs", f"Select a device.", choices)
        return False
    ...
    dispatcher.send_blocks(
        dispatcher.command_response_header(
            "cloudvision", "get-device-bugs", [("Device Name", device_name)], "information"
        )
    )
