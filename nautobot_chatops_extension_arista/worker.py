...
def check_credentials(dispatcher):
    if not os.getenv("NAUTOBOT_CHATOPS_CVAAS_TOKEN"):
        if not os.getenv("CVP_USERNAME") and not os.getenv("CVP_PASSWORD") and not os.getenv("CV_HOSTNAME_OR_IP"):
            dispatcher.send_warning("Please ensure either environment variable NAUTOBOT_CHATOPS_CVAAS_TOKEN is set or environment variables CVP_USERNAME, CVP_PASSWORD and CVP_HOSTNAME_OR_IP are set.")
        return False
    return True
...
def get_devices_in_container(dispatcher, container_name=None):
    """Get list of devices in a Cloudvision container."""
    ...
    if not container_name:
        ...
        dispatcher.prompt_from_menu(
            f"cloudvision get-devices-in-container", "Select Container", choices
        )
        return False
...
def get_configlet(dispatcher, configlet_name=None):
    """Get configuration of specific configlet."""
    ...
    if not configlet_name:
        ...
        dispatcher.prompt_from_menu(
            "cloudvision get-configlet", "Select Configlet", choices
        )
        return False
    ...
    dispatcher.send_blocks(
        dispatcher.command_response_header(
            "cloudvision", "get-configlet", [("Configlet Name", configlet_name)]
        )
    )
...
def get_device_configuration(dispatcher, device_name=None):
    ...
    if not device_name:
        ...
        dispatcher.prompt_from_menu(
            f"cloudvision get-device-configuration", "Select Device", choices
        )
    ...
    dispatcher.send_blocks(
        dispatcher.command_response_header(
            "cloudvision", "get-device-configuration", [("Device Name", device_name)]
        )
    )
...
def get_task_logs(dispatcher, task_id=None):
    ...
    if not task_id:
        ...
        dispatcher.prompt_from_menu(
            f"cloudvision get-task-logs", "Select task", choices
        )

        return False

    dispatcher.send_markdown(
        f"Stand by {dispatcher.user_mention()}, I'm getting the logs of task {task_id}."
    )
    ...
    if not single_task_cc_id:
        dispatcher.send_warning(
            f"No ccIdV2 for task {task_id}. The task was likely cancelled."
        )
        return CommandStatusChoices.STATUS_FAILED
    ...
    dispatcher.send_blocks(
        dispatcher.command_response_header(
            "cloudvision", "get-task-logs", [("Task ID", task_id)]
        )
    )
...
def get_applied_configlets(dispatcher, filter_type=None, filter_value=None):
    ...
    if not filter_value:
        ...
        if not choices:
            dispatcher.send_error(f"No data found to filter by.")
    ...
    if not applied_configlets:
        dispatcher.send_warning(
            f"There are no configlets applied to {filter_type} {filter_value}."
        )
        return CommandStatusChoices.STATUS_FAILED
...
def get_active_events(dispatcher, filter_type=None, filter_value=None, start_time=None, end_time=None):
    """Get active events from CVP based on severity or device. Can also get 'all` active events."""
    ...
    if not filter_value:
        if filter_type == "severity":
            ...
            choices = [(device["result"]["value"]["hostname"], device["result"]["value"]["hostname"]) for device in device_list]
        ...
        dispatcher.prompt_from_menu(
            f"cloudvision get-active-events {filter_type}", f"Select a {filter_type}.", choices
        )
        return False

    if not start_time:
        dispatcher.prompt_for_text(f"cloudvision get-active-events {filter_type} {filter_value}", "Enter start time in ISO format.", "Start Time")
        return False

    if not end_time:
        dispatcher.prompt_for_text(f"cloudvision get-active-events {filter_type} {filter_value} {start_time}", "Enter end time in ISO format.", "End Time")
        return False
    ...
    if filter_type == "severity":
        active_events = get_active_events_data_filter(filter_type=filter_type, filter_value=filter_value, start_time=start_time, end_time=end_time)
        dispatcher.send_markdown(
    ...
    dispatcher.send_markdown(
        f"Stand by {dispatcher.user_mention()}, I'm getting those events."
    )
    ...
    dispatcher.send_blocks(
        dispatcher.command_response_header(
            "cloudvision",
            "get-active-events",
            [("Filter type", filter_type), ("Filter value", filter_value), ("Start time", str(start_time)), ("End time", end_time)],
            "information"
        )
    )
...
def get_applied_image_bundles(dispatcher, filter_type=None, image_bundle_name=None):
    ...
    if filter_type == "all":
        dispatcher.send_markdown(
            f"Stand by {dispatcher.user_mention()}, I'm getting all image bundles in Cloudvision."
        )
        image_bundles = get_image_bundle()
    ...
    if not image_bundle_name:
        ...
        dispatcher.prompt_from_menu(
            f"cloudvision get-applied-image-bundles {filter_type}", f"Select an image bundle.", choices
        )
        return False
    ...
    if not applied_dictionary["containers"] and not applied_dictionary["devices"]:
        dispatcher.send_warning(
            f"There are no devices or containers with {image_bundle_name} applied."
        )
        return CommandStatusChoices.STATUS_FAILED
...
def get_device_cve(dispatcher, device_name=None):
    """Get CVEs Cloudvision has found for a device or a list of all devices with CVEs."""
    ...
    if not device_name:
        ...
        dispatcher.prompt_from_menu(
            f"cloudvision get-device-bugs", f"Select a device.", choices
        )
        return False
    ...
    if device_name == "all":
        ...
        dispatcher.send_markdown(
            f"Stand by {dispatcher.user_mention()}, I'm getting that CVE report for you."
        )
        ...
        rows = [
            (
                get_cloudvision_devices_by_sn(device_sn),
                bug_count
            )
            for device_sn,bug_count in bug_count.items()
        ]
    ...
    if not device_bugs:
        dispatcher.send_warning(
            f"There are no bugs for device {device_name}."
        )
        return CommandStatusChoices.STATUS_FAILED
...
@subcommand_of("cloudvision")
def get_compliant_devices():
    """Get compliant status of devices."""
